...
## 1. 概要
本リポジトリでは、2023年度のProject PLATEAUで開発した「公園管理台帳システム」のソースコードを公開しています。  
「公園管理台帳システム」は、公園施設を管理するための台帳システムです。3D都市モデル（都市設備モデル、植生モデル、徒歩道モデル等）をデータベース管理することで、点検管理業務の効率化を図ります。  
また、本システムは、スマートフォン向けの点検アプリとの連携機能や、2D/3Dの地図表示機能等を備えており、簡易なユーザーインターフェースを実現しています。  

本システムはCARTO等の有償ライセンスを前提としたクラウドシステムです。

## 2. 「公園管理台帳システム」について
「公園管理台帳システム」は、3D 都市モデルを活用した公園管理用のデータベースシステムです。  
公園管理における施設情報の管理、点検記録の管理、「公園施設長寿命化計画」に基づく方針立案や維持保全活動の情報共有等に活用することを目的としています。
なお、本システムは、スマートフォン向け管理用アプリケーションと併せた利用を想定していますが、こちらは本リポジトリには含まれていません。
